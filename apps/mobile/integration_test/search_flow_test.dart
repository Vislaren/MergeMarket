// Integration test for the B-03 search flow (USER_FLOWS Flow 2).
//
// Drives the REAL app end-to-end against a running B-02 mock server: type a
// query on Home → navigate to Results → see the multi-store list sorted by
// total cost. No mocks — this exercises the live HTTP repository.
//
// Requires:
//   1. A connected device/emulator (or `-d chrome`).
//   2. The mock server running and reachable at the configured base URL.
//        cd services/mock-server && MOCK_SERVER_PORT=8081 go run ./cmd/mock-server
//   3. The base URL passed in (Android emulator reaches the host at 10.0.2.2):
//        flutter test integration_test/search_flow_test.dart \
//          --dart-define=API_BASE_URL=http://10.0.2.2:8081
//
// Status: PENDING — no device/emulator available in the dev environment this
// session. The suite compiles and is ready to run once a device + mock server
// are present.

import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:integration_test/integration_test.dart';
import 'package:mergemarket/main.dart';
import 'package:mergemarket/widgets/mm_product_card.dart';
import 'package:mergemarket/widgets/mm_search_bar.dart';

void main() {
  IntegrationTestWidgetsFlutterBinding.ensureInitialized();

  group('B-03 Search flow integration tests', () {
    testWidgets('TC-06-I-001: search from Home renders store results',
        (tester) async {
      await tester.pumpWidget(const ProviderScope(child: MergeMarketApp()));
      await tester.pumpAndSettle();

      // Type a query into the search bar and submit.
      await tester.enterText(find.byType(MMSearchBar), 'galaxy');
      await tester.testTextInput.receiveAction(TextInputAction.search);
      await tester.pumpAndSettle(const Duration(seconds: 5));

      // Results screen shows at least one store offer card.
      expect(find.byType(MMProductCard), findsWidgets);
      // Total cost figure is rendered (currency prefix present).
      expect(find.textContaining('XAF'), findsWidgets);
    });

    testWidgets('TC-06-I-002: tapping a result opens Product Detail',
        (tester) async {
      await tester.pumpWidget(const ProviderScope(child: MergeMarketApp()));
      await tester.pumpAndSettle();

      await tester.enterText(find.byType(MMSearchBar), 'galaxy');
      await tester.testTextInput.receiveAction(TextInputAction.search);
      await tester.pumpAndSettle(const Duration(seconds: 5));

      await tester.tap(find.byType(MMProductCard).first);
      await tester.pumpAndSettle();

      // Product Detail route reached (real screen lands in B-04).
      expect(find.textContaining('prod-'), findsWidgets);
    });
  });
}
